...
function iniciarBusca() {
    ...
    let dadosFiltrados = dados.filter(dado => {
        // Converte todos os campos de texto do card para minúsculas para uma busca case-insensitive
        const mes = dado.mês.toLowerCase();
        const descricao = dado.descricao.toLowerCase();

        // Verifica se o termo de busca está no mês ou na descrição
        if (mes.includes(termoBusca) || descricao.includes(termoBusca)) {
            return true;
        }

        // Verifica se o termo de busca corresponde a alguma música
        const temMusica = dado.musicas.some(musica => musica.nome.toLowerCase().includes(termoBusca));
        if (temMusica) { // Se encontrou na música, já retorna true
            return true;
        }

        // Verifica se o termo de busca corresponde a algum artista
        const temArtista = dado.artistas.some(artista => artista.nome.toLowerCase().includes(termoBusca));
        return temArtista; // Se não encontrou antes, o resultado final depende de ter encontrado no artista
    });
